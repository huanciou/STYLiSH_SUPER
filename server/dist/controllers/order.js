...
async function payByPrime({ prime, recipient, amount, details, orderNumber, }) {
...
async function checkProducts(inputList) {
    ...
    const checkProductVariant = (product) => {
        ...
        const targetVariant = variants.find((v) => {
            return (v.color_name === product.color.name &&
...
async function placeOrder({ userId, orderInfo, recipient, products, connection, }) {
    const { shipping, payment, subtotal, freight, total } = orderInfo;
    connection.query("BEGIN");
    try {
        const { orderId, orderNumber } = await orderModel.createOrder(userId, {
            shipping,
            payment,
            subtotal,
            freight,
...
async function confirmOrder({ orderId, orderNumber, amount, prime, products, recipient, connection, }) {
    try {
        connection.query("BEGIN");
        const variantIds = products.map(({ variantId }) => variantId);
        const variants = await getVariantsStockWithLock(variantIds, connection);
        const variantsMapWithNewStock = products.reduce(function (variantsMap, product) {
