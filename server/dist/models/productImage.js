...
*/
export async function createProductImages(imageData) {
    try {
        await pool.query(`
      ...
      `, [
            imageData.map((data) => {
                const { productId, originalname, mimetype, filename, path, size, fieldname, } = data;
                return [
                    productId,
                    originalname,
                    mimetype,
...
export function groupImages(productImages) {
    const result = productImages.reduce(function (obj, image) {
