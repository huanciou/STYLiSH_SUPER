...
export async function getProducts({ paging = 0, category, }) {
...
export async function searchProducts({ paging = 0, keyword, }) {
    const results = await pool.query(`
    SELECT * FROM products
    WHERE title LIKE ?
    ORDER BY id DESC
...
function instanceOfProductCount(object) {
    return "count" in object;
}
export async function countProducts({ category, keyword, }) {
    const results = await pool.query(`
...
export async function createProduct(productData) {
    try {
        const { category, title, description, price, texture, wash, place, note, story, } = productData;
        const results = await pool.query(`
...
export async function isProductExist(productId) {
    const results = await pool.query(`
